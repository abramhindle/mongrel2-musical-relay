...
sub filterit {
        ...
        for my $elm (@$arr) {
            ...
            my $loudness = $maxloud / 10.0 +  10*$maxloud*$dist / $maxdist;
            my $pitch = 20 + 40*(100 + exp(log(1000)*$i/100.0) + exp(log(10000)*$dist/$maxdist) + exp(log(2000)*abs(500-$x)/500.0) + -1000*$v + 1000*$w)/10000;
